...
impl SessionInitializer for ChromeSessionInitializer {
    ...
    async fn create_sessions<'a>(
        ...
        for i in params.iter() {
            let driver = DriverSession::new::<<Self::Service as DriverService>::BrowserConfigType>(host, port.as_str(), i.as_str()).await;
            results.push(driver);
        }
        results
    }
...
impl SessionInitializer for GeckoSessionInitializer {
    ...
    async fn create_sessions<'a>(
        ...
        for i in ports {
            ...
            let driver =
                DriverSession::new::<<Self::Service as DriverService>::BrowserConfigType>(host, port_str.as_str(), base64_profile).await;
            results.push(driver);
        }
        results
    }
